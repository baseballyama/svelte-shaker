...
> residual・dedup マップ・測定ベースの純減ゲート**を計算する。`shaker({ level: 2, monomorphize: true })`
> （Vite/Rollup）または `svelteShakerWithMono(entries, …, { enabled: true, maxVariants, minSavings })` で
> **opt-in**。既定は OFF で**完全に byte 一致**（挙動不変）。
>
...
- **M6｜L2 コールサイト・モノモーフィズ（opt-in、測定ベース net-win ゲート）** ✅：エンジン `src/mono.ts`
...
'client'})` の `js.code.length`、メモ化）で測り、**`Σ_spec < Σ_base * (1 - minSavings)` の厳密純減のときだけ**
  ...
  `tests/mono.test.ts` で担保。`shaker({ level:2, monomorphize:true })` で有効化（既定 OFF）。
