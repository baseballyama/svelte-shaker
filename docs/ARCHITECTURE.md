...
- **ネイティブエンジンの配布**：per-platform prebuilt `.node`（`@napi-rs/cli`）+ `optionalDependencies`
  公開 + release ワークフローの platform matrix。**未着手**——これが繋がるまで npm インストールには
  ネイティブバイナリが無く、`tryLoadNativeEngine` が null を返して WASM/JS にフォールバックする（挙動は
  従来どおり byte 一致）。ローダ（`engine-scan-native/index.cjs`）は published `.node` の解決を実装済み。
