...
#### L2 真のモノモーフィズ（opt-in、さらに攻める）

L1.5 で削った後、`<Button variant="primary">` の**呼び出しごと**に `variant='primary'` 固定の複製を作り
`variant==='primary'` を true 畳みして primary 以外を全消し。最強だが形状数だけ複製が増える。
`virtual:shaker/Button?variant=primary` のような仮想モジュールへ import を張り替え、同形状を dedup し、
`maxVariants` でサイズ回帰をガードする。**opt-in** とする。

> **実装状況（M6 / 実装済み）**：エンジン（`src/mono.ts`）が **コールサイトごとの特殊化 residual と
> dedup マップ**を計算する。`shaker({ level: 2, monomorphize: true })`（Vite/Rollup）または
> `svelteShakerWithMono(..., { enabled: true, maxVariants })` で **opt-in**。既定は OFF で挙動不変。
>
...
> - **サイズガード**：最小でも dedup。加えて `maxVariants`（コンポーネント単位、既定 8）で複製数を上限。
>   超過したサイトはベース（無特殊化）にフォールバック＝常に健全。`minSavings`（測定ベースの厳密最適抽出、
>   §13.2）は未実装で、dedup + cap の保守的ガードに留める（後続）。
...
>   （`tests/mono.test.ts`：L1/L1.5 が消せない分岐を L2 が消す + 等価性 + dedup + cap + bail）。
...
- **L0 / L1 / L1.5 = 既定で有効、L2 = opt-in** ✅：§3 の表のとおり。`level` 未指定でも挙動不変。
- **M6｜L2 コールサイト・モノモーフィズ（opt-in）** ✅：エンジン `src/mono.ts`（生きたサイト × spread に
  上書きされ得ないリテラル prop のみ特殊化、residual は L0/L1/L1.5 と同一の `shakeBody` で生成）＋
  仮想モジュール配線（`?shaker_variant=<n>`）＋ residual dedup ＋ `maxVariants` cap。差分 SSR 等価・dedup・
  cap・bail を `tests/mono.test.ts` で担保。`shaker({ level:2, monomorphize:true })` で有効化（既定 OFF）。

### REMAINING（未実装・後続）

- **L2 フォローアップ**：`minSavings` の測定ベース最適抽出（§13.2、現状は dedup + `maxVariants` cap の保守ガード）、
  Rollup 経路の L2 e2e テスト。`exclude` / `unsafe` / `report` の API は予約のみ（未実装）。
