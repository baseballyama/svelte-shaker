...
- ¹ **L2 は Vite プラグインでは既定 ON**（net-win ゲートで肥大しないため。下記）。`level: 1`／
...
- **M0｜骨格** ✅：IR 確定（§5.1 / `src/ir.ts`）。Engine（`src/{parse,analyze,eval,dead,transform,css,index}.ts`）/
  Shell（`src/vite.ts`）分離。`basic1` を L0/L1 で緑に（新既定＝署名まで縮める）。
...
- **L0 / L1 / L1.5 / L2 = Vite プラグインで既定有効** ✅：§3 の表のとおり。`level` 未指定で L2 まで
