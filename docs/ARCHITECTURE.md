...
> dead code をツリーシェイクしてから Svelte コンパイラへ渡す Vite / Rollup プラグイン。
...
| -------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ------ |
| **L0**   | 未使用 prop 除去             | どのサイトからも渡されない prop を default で畳んで DCE。**`$props()` の署名からも落とす**                          | 1/コンポーネント              | ✅     |
...
| **L2**   | コールサイト・モノモーフィズ | prop 形状ごとにコンポーネントを複製・特殊化                                                                         | N/形状（dedup・サイズガード） | ✅¹    |

- **L0/L1/L1.5 が本命**。1 コンポーネント 1 モジュールを保つので安全・低コスト・コードサイズ増なし。
- ¹ **L2 は Vite/Rollup プラグインでは既定 ON**（net-win ゲートで肥大しないため。下記）。`level: 1`／
...
#### L2 真のモノモーフィズ（**既定 ON**、**測定ベースの純減ゲートで「絶対に肥大させない」**）
...
> L2 は各サイトを特殊化（`a` か `b` が定数化）→ **どちらの variant でも `{#if a===1&&b===1}` が false に
...
import は `virtual:shaker/Button?shaker_variant=<n>` 相当の仮想モジュールへ張り替え、同形状は dedup する。
**Vite/Rollup プラグインでは既定 ON**（肥大しないため）。

> **実装状況（M6 + net-win ゲート / 実装済み）**：エンジン（`src/mono.ts`）が **コールサイトごとの特殊化
> residual・dedup マップ・測定ベースの純減ゲート**を計算する。**Vite/Rollup プラグインでは既定 ON** で、
> `shaker({ level: 1 })`／`shaker({ monomorphize: false })` で OFF（ビルドを速くする）。環境フリーの
> エンジン API（`svelteShakerWithMono(entries, …, { enabled: true, maxVariants, minSavings })`）は既定 OFF で、
> OFF のときは L1.5 出力と**完全に byte 一致**（挙動不変）。
>
...
>   （escape/barrel/accessors）・shadow される prop・`{@debug}` prop・L1 で既に畳んだ prop は
...
>   却下する + L2 OFF byte 一致 + 差分 SSR 等価 + dedup + all-sites cap + bail + e2e で L2 バンドル ≤ L1.5）。
...
packages/
  ...
  rollup-plugin-svelte-shaker/   … Shell（Rollup）
  vite-plugin-svelte-shaker/     … Shell（Vite。enforce:'pre'、2 パス、HMR ポリシー）
  example/                       … 動作確認・回帰フィクスチャ
```

- 第一段階は **L0 + L1**、`basic1` を緑にするところから。`svelte/compiler` の `parse` と
...
```ts
shaker({
  level: 0 | 1 | 2, // 既定 2（L2 既定 ON。level:1/0 で OFF）
  include,
  exclude, // glob
...
**現状の実装サーフェス**（Vite `src/vite.ts` / Rollup `rollup-plugin-svelte-shaker`）：

```ts
// L0/L1/L1.5 + L2（既定。L2 は net-win ゲートで肥大しないので既定 ON）
shaker({ include: ['.'] });

// L2 を OFF（ビルドを速くする。圧縮率は少し落ちる）
shaker({ include: ['.'], level: 1 });
...
`minSavings` は**実装済み**（既定 0＝厳密純減のみ。§3 L2 / §13.2 の測定ベース net-win ゲート）。L2 のサイズ
...
- **M0｜骨格** ✅：IR 確定（§5.1 / `src/ir.ts`）。Engine（`src/{parse,analyze,eval,dead,transform,css,index}.ts`）/
  Shell（`src/vite.ts`・`rollup-plugin-svelte-shaker`）分離。`basic1` を L0/L1 で緑に（新既定＝署名まで縮める）。
  dev は素通し（`apply:'build'`）。
- **M1｜実用 L0/L1 + 部分 bail** ✅：whole-program fixpoint カスケード（`analyze.ts` + `dead.ts` の単一述語
  ...
  サージカル span 削除。
- **M2｜L1.5 値集合ナローイング + 堅牢化** ✅：`multi`（値集合）抽象、到達不能な if/else-if アームの除去
...
- **L0 / L1 / L1.5 / L2 = Vite/Rollup プラグインで既定有効** ✅：§3 の表のとおり。`level` 未指定で L2 まで
  ON（net-win ゲートで肥大しない）。`level: 1`／`monomorphize: false` で L2 OFF（ビルド速度トレードオフ）。
- **M6｜L2 コールサイト・モノモーフィズ（既定 ON、測定ベース net-win ゲート）** ✅：エンジン `src/mono.ts`
  （生きたサイト × spread に上書きされ得ないリテラル prop のみ特殊化、residual は L0/L1/L1.5 と同一の
...
'client'})` の `js.code.length`、メモ化）で測り、**`Σ_spec < Σ_base * (1 - minSavings)` の厳密純減のときだけ**
  特殊化（§3 L2 / §13.2）。`maxVariants`（既定 8）超過の子は全サイト特殊化不能＝丸ごとベース維持。
  ...
  却下 / L2 OFF byte 一致 / 差分 SSR 等価 / dedup / all-sites cap / bail / e2e で L2 バンドル ≤ L1.5 を
  `tests/mono.test.ts` で担保。プラグインでは既定 ON（`shaker({ level: 1 })` で OFF）。エンジン API
  `svelteShakerWithMono` は `{ enabled: true }` で有効化（既定 OFF）。

### REMAINING（未実装・後続）

- **L2 フォローアップ**：測定ベースの **net-win ゲート（all-sites-or-nothing ＋ `Σ_spec < Σ_base * (1-minSavings)`）
  ...
  コストモデル、(c) Rollup 経路の L2 e2e テスト。`exclude` / `unsafe` / `report` の API は予約のみ（未実装）。
- **M3 残**：大規模デザインシステムでのベンチ、`svelte-package` 配布ライブラリでの実測、CI での
  「shake 有/無の prod ビルド同値」回帰の常設。
- **M4｜Rust 化①**：パーサを rsvelte（OXC）napi に差し替え（§9-1）。**未着手**（現行は `svelte/compiler` parse）。
...
2. **prop 宣言の扱い → 署名まで縮める（攻め）**。未使用／定数畳み済み prop は `$props()` から落とす。
   L1 では連動して全コールサイトの該当属性も除去（副作用を持つ属性式は保持）。
...
7. **L2 複製の判断 → `maxVariants`/`minSavings` を廃し、§13 の原理的機構で確定**。
