...
| **コールサイトの非 prop 属性** `<Comp --accent="red" my:directive />`                          | その属性のみ                 | prop ではないので reverse 除去の対象外。`--*` は `$.css_props` が `<svelte-css-wrapper>` 要素を描画する**マークアップ**、未知 namespace の `ns:x` はパーサが Directive 化しない生の属性（既知 prefix のみ Directive 化）でプリプロセッサ向けマーカー。コンポーネント自体は通常どおり shake する（escape ではない） |
...
ネイティブエンジン（`engine-scan-native`、`src/native-engine.ts` が駆動）は、AST を一切境界に通さず
**edits だけ**を返す常駐 Session 方式で動く。JS↔native の往復は「chatty protocol」——大きな AST では
なく、facts・resolve 済み edges・edits という小さなペイロードだけを渡す。

1. **`session.parse(seed)`** — seed（`collectSvelteFiles` が集めた `.svelte`）を rsvelte で**一括・
   並列**パースし、各ファイルの AST を Session に保持したまま、Round-1 の facts
   （import specifier / rendered `<Local>` tag / member `<ns.X>` tag）を返す。
2. **facts-provider crawl** — `buildAnalyzeInput` の crawl は、ファイルごとに JS で再パースする代わりに
...
> この時点（案）の `unsafe` と `report` はその後の実装では姿を変えた：`unsafe` は不採用のまま終わり、
> `report` は `verbose` に改名されて実装済み（`packages/svelte-shaker/src/vite.ts` の
> `ShakerOptions.verbose`）。一方この案にはなかった `exclude`（生成物ディレクトリの剪定。後述）が
> 別途採用され実装済み。現状のキー集合は §11 DONE および `src/vite.ts` を正とする。
...
`exclude`（生成物ディレクトリの剪定。§8.1.1 後段）は**実装済み**。`unsafe`（bail を緩める脱出口）は
不採用のまま終わった。`report` は `verbose`（per-file size breakdown の出力）に改名の上で**実装済み**
（§11 DONE）。
...
- **escapeScan の native 化**：非 `.svelte` の `.ts`/`.js` モジュール import 抽出を native 化する案は
  ROI 不足でバックログ（~0.3s、新規 native 能力が必要）。1s の最終詰めで必要になれば escaped byte 等価
  pin 付きで再評価。
...
- **monomorphization フォローアップ**：測定ベースの **net-win ゲート（all-sites-or-nothing ＋ `Σ_spec < Σ_base * (1-minSavings)`）
  ...
  コストモデル。（`exclude` は実装済み、`unsafe` は不採用、`report` は `verbose` に改名の上で実装済み — §8.1 参照）
